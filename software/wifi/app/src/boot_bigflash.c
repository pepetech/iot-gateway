...
void Cache_Read_Enable_New()
{
	...
	if (mmap_1 == 0xFF)
	{
		...
		uint32_t off = (uint8_t*)&rtc_conf.last_rom - (uint8_t*)&rtc_conf;

		uint32_t val = *(volatile uint32_t*)(0x60001100 + (BOOT_RTC_ADDR * 4) + (off & ~((uint32_t)3)));
		val = conf.roms[((uint8_t*)&val)[off & (uint32_t)3]];
