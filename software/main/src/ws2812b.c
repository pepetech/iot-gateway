...
void ws2812b_init()
{
    ...
    pDMADescriptor[0].IMMVAL = TIMER_ROUTEPEN_CC0PEN;
    pDMADescriptor[0].DST = (void *)PERI_REG_BIT_SET_ADDR(&(TIMER1->ROUTEPEN));
    pDMADescriptor[0].LINK = 0x00000010 | LDMA_CH_LINK_LINK | LDMA_CH_LINK_LINKMODE_RELATIVE;

    pDMADescriptor[1].CTRL = LDMA_CH_CTRL_DSTMODE_ABSOLUTE | LDMA_CH_CTRL_SRCMODE_ABSOLUTE | LDMA_CH_CTRL_DSTINC_NONE | LDMA_CH_CTRL_SIZE_HALFWORD | LDMA_CH_CTRL_SRCINC_ONE | LDMA_CH_CTRL_REQMODE_BLOCK | LDMA_CH_CTRL_BLOCKSIZE_UNIT1 | ((((WS2812B_NUM_LEDS * 3 * 8) - 1) << _LDMA_CH_CTRL_XFERCNT_SHIFT) & _LDMA_CH_CTRL_XFERCNT_MASK) | LDMA_CH_CTRL_STRUCTTYPE_TRANSFER;
    ...
    pDMADescriptor[2].IMMVAL = TIMER_ROUTEPEN_CC0PEN;
    pDMADescriptor[2].DST = (void *)PERI_REG_BIT_CLEAR_ADDR(&(TIMER1->ROUTEPEN));
...
void ws2812b_set_color(uint16_t usLED, uint8_t ubRed, uint8_t ubGreen, uint8_t ubBlue)
{
    ...
    if(ldma_ch_get_busy(WS2812B_DMA_CHANNEL))
        return;
