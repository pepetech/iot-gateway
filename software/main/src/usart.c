		...
		while(ulSize--)
		{
			if(pubDst)
				*(pubDst++) = usart2_spi_transfer_byte(*(pubSrc++));
				...
				usart2_spi_transfer_byte(*(pubSrc++));
		}
	}
	else if(pubDst)
	{
		while(ulSize--)
			*(pubDst++) = usart2_spi_transfer_byte(0x00);
	}
}
...
static void usart2_dma_isr(uint8_t ubError)
{
    ...
    if(pubDMANextDst != (pubUSART2DMABuffer + (USART2_DMA_RX_BUFFER_SIZE >> 1)) && pubDMANextDst != pubUSART2DMABuffer)
        DBGPRINTLN_CTX("WARNING: DMA dst is not aligned to any buffer 0x%08X != 0x%08X && 0x%08X != 0x%08X", pubDMANextDst, (pubUSART2DMABuffer + (USART2_DMA_RX_BUFFER_SIZE >> 1)), pubDMANextDst, pubUSART2DMABuffer);
...
void usart2_init(uint32_t ulBaud, uint32_t ulFrameSettings, int8_t bRXLocation, int8_t bTXLocation, int8_t bCTSLocation, int8_t bRTSLocation)
{
    ...
    USART2->CTRL = USART_CTRL_TXBIL_EMPTY | USART_CTRL_CSMA_NOACTION | UART_CTRL_OVS_X16;
